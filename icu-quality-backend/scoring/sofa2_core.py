...
def _calc_respiratory(
    ...
    # 先查 P/F ratio
    val, unit, ts = _worst_in_window(obs, ["param_bg_P/Fratio"], eval_time, 24, 4)
    if val is not None and val > 0:
        ratio = val
    else:
        val_pao2, _, ts_pao2 = _worst_in_window(obs, ["param_PaO2", "PaO2"], eval_time, 24, 4)
        val_fio2, _, ts_fio2 = _worst_in_window(obs, ["param_FiO2", "FiO2"], eval_time, 24, 4)
        if val_pao2 is not None and val_fio2 is not None and val_fio2 > 0:
            ...
            val_spo2, _, _ = _worst_in_window(obs, ["SpO2", "param_SpO2"], eval_time, 24, 4)
...
def _parse_gcs(gcs_val: Any) -> Tuple[Optional[int], Optional[str]]:
    ...
    if isinstance(gcs_val, str):
        ...
        if m:
            ...
            v = 4 if v_raw.upper() == "T" else int(v_raw)
            return e + v + m_score, None
...
def _calc_kidney(
    ...
    # 尿量 (mL/kg/h, 需求文档 7.3 第 6 条: 粗粒度24h标记)
    val, unit, _ = _worst_in_window(
